...
SOURCES  = src/showVersion.ml src/version.ml src/regCred.ml
EXEC     = showVersion

CAMLC	 = ocamlc				# bytecode compiler
CAMLOPT  = ocamlopt				# native code compiler
CAMLDEP  = ocamldep
CAMLLEX  = ocamllex
CAMLYACC = ocamlyacc

LIBS= $(WITHSTR)
CUSTOM=-custom					# am using LIBS
WITHSTR =str.cma				# for regex

# ###################################################################
# Generic from here, or so we hope
# ###################################################################
#
...
.depend.input: Makefile
	...
	@(ls $(SMLIY) $(SMLIY:.ml=.mli) 2>/dev/null || true)  >  .depend.new
